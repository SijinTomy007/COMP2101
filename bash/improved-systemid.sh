...
# the LAN info uses a hardcoded interface name of "ens33"#    - change ens33 to whatever interface you have and want to gather info about
...
do
  ...
  # the LAN name is looked up using the LAN address in case it is different from the system name# the lan address is pulled out of the ip address command output
  ...
  # the router name is obtained with getentdefault_router_address=$(ip r s default| cut -d ' ' -f 3)
  ...
  # finding external information relies on curl being installed and relies on live internet connectionexternal_address=$(curl -s icanhazip.com|awk '{print $1}')
  ...
  External Name : $external_nameInterface Address         : $interface_ipv4_address
